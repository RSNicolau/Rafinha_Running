...
function TSBLabel({ tsb }: { tsb: number }) {
  if (tsb > 5)  return <span className="text-emerald-600 font-semibold">🟢 Fresco</span>;
  if (tsb > -10) return <span className="text-amber-600 font-semibold">🟡 Moderado</span>;
  return <span className="text-red-600 font-semibold">🔴 Fatigado</span>;
}
...
export default function TrainingLoadChart({ athleteId }: Props) {
  ...
  useEffect(() => {
    setLoading(true);
    api.get(`/workouts/training-load/${athleteId}?days=${days}`)
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <div className="glass-card p-4 text-center">
          ...
          {latest && <div className="text-xs mt-0.5"><TSBLabel tsb={latest.tsb} /></div>}
        </div>
      </div>
      ...
      <div className="glass-card p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-4 text-xs text-gray-500">
          <span><span className="inline-block w-3 h-0.5 bg-blue-500 mr-1 align-middle" />CTL = Fitness (média 42 dias)</span>
          <span><span className="inline-block w-3 h-0.5 bg-red-500 mr-1 align-middle" />ATL = Fadiga (média 7 dias)</span>
          <span><span className="inline-block w-3 h-0.5 bg-emerald-500 mr-1 align-middle" />TSB = Forma (CTL − ATL)</span>
        </div>
      </div>
    </div>
  );
